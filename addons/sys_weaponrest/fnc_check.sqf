...
if ((_currentAnim in __prone) || (_currentAnim in __pronedown)) exitwith {
	...
	// Bipods
	if ((__check == 1 || "muzzle_bipod_tup" in (primaryWeaponItems player) || "muzzle_bipod_tup" in (primaryWeaponItems player)) && (_currentAnim in __prone)) then {
		TRACE_2("Bipod Check",__check, primaryWeapon player);
		[__animprone_bipod,_currentAnim, localize "STR_ALIVE_BIPOD", false] call FUNC(bipod);
	};
	...
	if ([player,1.7] call FUNC(sb)) then {
		[_anim,_currentAnim, localize "STR_ALIVE_DEPLOYED", true] call FUNC(bipod);
	} else {
		if (__check != 1) then {
...
if !([_currentAnim] call FUNC(canRestWeapon)) exitwith { hint "Can't rest the weapon here"};
...
// Bipods
if (__check == 1 || "muzzle_bipod_tup" in (primaryWeaponItems player) || "muzzle_bipod_tup_m" in (primaryWeaponItems player)) exitwith {
	TRACE_2("Bipod Check",__check, primaryWeapon player);
	[_anim,_currentAnim, localize "STR_ALIVE_BIPOD", false] call FUNC(bipod);
	addCamShake [5, 1, 5];
};
...
while {(animationstate player) == _anim } do {
	...
	if (abs _res > 30) exitwith { player switchmove _oldanim;};
	sleep 0.5;
};
TRACE_3("UNRESTING WEAPON", abs _res, animationstate player, _anim);
